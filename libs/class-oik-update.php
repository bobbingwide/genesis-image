...
define( "CLASS_OIK_UPDATE_INCLUDED", "3.0.1" );
...
static function oik_query_plugins_server( $slug ) {
	...
	if ( $plugin_settings ) {
		...
		if ( !$server ) {   
			$server = bobbcomp::bw_array_get_dcb( $value, "server", null, "oik_plugins::oik_get_plugins_server" );
		}
		if ( !$apikey ) {
		...
		if ( $plugin_settings ) {
				$server = bobbcomp::bw_array_get_dcb( $plugin_settings, "server", null, "oik_plugins::oik_get_plugins_server" );
		}
